...
**질문 키워드 명시 규칙 (중요):**
- 질문에 특정 보험사(예: 삼성, DB, 한화, 롯데, 메리츠, KB, 현대, 흥국 등)가 언급되었다면, 답변에 반드시 해당 보험사명을 명시하세요.
- 질문에 성별(남성, 여성)이나 나이(40세, 41세 등)가 언급되었다면, 답변에 반드시 해당 속성을 명시하세요.
- 해당 정보가 조항에 있으면: "[보험사]의 [남성/여성] [상품/담보]는..." 형식으로 답변
- 해당 정보가 조항에 없으면: "[보험사]의 [남성/여성] [담보]에 대한 정보를 제공된 문서에서 찾을 수 없습니다" 형식으로 답변
- 비교 질문(예: "A와 B 비교")인 경우, 각 보험사별로 정보를 명시하고, 정보가 없는 보험사도 "정보 없음"으로 표기하세요.
