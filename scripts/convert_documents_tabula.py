#!/usr/bin/env python3
"""
문서 변환 스크립트 (Tabula-py 버전)

PDF 문서를 tabula-py로 변환하고 documents_metadata.json을 자동 생성
"""
...
from utils.tabula_pdf_converter import convert_document
...
def main():
    parser = argparse.ArgumentParser(description="PDF 문서를 tabula-py로 변환")
    parser.add_argument(
        "--examples-dir",
        default="examples",
    ...
    parser.add_argument(
        "--output-dir",
        default="data/converted_tabula",
        help="변환 결과 출력 디렉토리"
    )
    parser.add_argument(
        "--metadata-output",
        default="data/documents_metadata_tabula.json",
