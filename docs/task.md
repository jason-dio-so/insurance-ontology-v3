# Database Refactoring Tasks

**시작일**: 2025-12-13
**목표 완료일**: 2025-12-27
**현재 상태**: 진행 중

---

## 상태 범례

| 상태 | 설명 |
|------|------|
| ⬜ PENDING | 아직 시작하지 않음 |
| 🔄 IN_PROGRESS | 진행 중 |
| ✅ COMPLETED | 완료됨 |
| ❌ FAILED | 실패 (원인 기록 필요) |
| ⏸️ BLOCKED | 다른 작업에 의해 차단됨 |

---

## 전체 Task 의존성 다이어그램

```
Phase 1 (스키마 추출) ✅
    ├── Phase 1.5a (PostgreSQL 설계 문서) ✅
    │       └── Phase 3 (스크립트/검증 도구)
    │               └── Phase 6 (CI/CD)
    ├── Phase 1.5b (Neo4j 설계 문서) ✅
    │       └── Phase 4 (Neo4j 스키마)
    │               └── Phase 7 (온톨로지 확장)
    ├── Phase 2 (Alembic) ✅
    │       └── Phase 2.5 (DB 전환) ✅
    │               └── Phase 3 (스크립트/검증 도구)
    └── Phase 5 (pgvector) [독립]
```

---

## 설계 → 구현 연결 관계

설계 문서와 구현 파일 간의 연결을 명확히 합니다.

### PostgreSQL

```
설계 문서                              구현 파일
─────────────────────────────────────────────────────────
db_refactoring/docs/
├── ER_DIAGRAM.md ─────────────────→ postgres/001_initial_schema.sql ✅
├── DATA_DICTIONARY.md                postgres/002_seed_data.sql ✅
├── RELATIONSHIP_MAP.md
└── DOMAIN_MODEL.md
```

### Neo4j

```
설계 문서                              구현 파일
─────────────────────────────────────────────────────────
db_refactoring/docs/
├── GRAPH_SCHEMA.md ───────────────→ neo4j/001_graph_schema.cypher ⬜ (Phase 4.1)
├── NODE_DICTIONARY.md                ingestion/graph_loader.py ✅
├── RELATIONSHIP_DICTIONARY.md
├── GRAPH_PATTERNS.md
└── ONTOLOGY_GAP_ANALYSIS.md
```

### 문서 인덱스

전체 문서 구조는 [`db_refactoring/docs/README.md`](../db_refactoring/docs/README.md) 참조

---

## Phase 1: 스키마 추출 (Day 1-2)

**목표**: 현재 PostgreSQL 스키마를 SQL 파일로 추출
**선행 조건**: 없음 (첫 번째 작업)
**후행 작업**: Phase 1.5, Phase 2, Phase 5

| ID | Task | 상태 | 선행 | 시작일 | 완료일 |
|----|------|------|------|--------|--------|
| 1.1 | 디렉토리 구조 생성 | ✅ COMPLETED | - | 2025-12-13 | 2025-12-13 |
| 1.2 | 현재 스키마 덤프 실행 | ✅ COMPLETED | 1.1 | 2025-12-13 | 2025-12-13 |
| 1.3 | `db_refactoring/postgres/001_initial_schema.sql` 정리 | ✅ COMPLETED | 1.2 | 2025-12-13 | 2025-12-13 |
| 1.4 | `db_refactoring/postgres/002_seed_data.sql` 생성 | ✅ COMPLETED | 1.2 | 2025-12-13 | 2025-12-13 |
| 1.5 | 스키마 파일 검증 (재적용 테스트) | ✅ COMPLETED | 1.3, 1.4 | 2025-12-13 | 2025-12-13 |

### Task 상세

#### 1.1 디렉토리 구조 생성

| 항목 | 내용 |
|------|------|
| 설명 | `db_refactoring/postgres/`, `db_refactoring/neo4j/`, `db_refactoring/docs/`, `db_refactoring/scripts/`, `db_refactoring/migrations/` 구조 생성 |
| 명령어 | `mkdir -p db/{postgres,neo4j,docs,scripts,migrations/versions}` |
| 완료 기준 | 모든 디렉토리 존재 확인 |

#### 1.2 현재 스키마 덤프 실행

| 항목 | 내용 |
|------|------|
| 설명 | pg_dump를 사용하여 현재 PostgreSQL 스키마 추출 |
| 명령어 | `pg_dump "postgresql://postgres:postgres@127.0.0.1:5432/insurance_ontology" --schema-only --no-owner --no-privileges > db_refactoring/postgres/001_initial_schema.sql` |
| 완료 기준 | SQL 파일 생성, 문법 오류 없음 |

#### 1.3 `db_refactoring/postgres/001_initial_schema.sql` 정리

| 항목 | 내용 |
|------|------|
| 설명 | 덤프 파일 정리 (주석 추가, 순서 정렬, 불필요 내용 제거) |
| 완료 기준 | 깔끔하게 정리된 스키마 파일 |

#### 1.4 `db_refactoring/postgres/002_seed_data.sql` 생성

| 항목 | 내용 |
|------|------|
| 설명 | coverage_category, disease_code_set 등 초기 시드 데이터 추출 |
| 완료 기준 | INSERT 문 포함된 시드 파일 생성 |

#### 1.5 스키마 파일 검증 (재적용 테스트)

| 항목 | 내용 |
|------|------|
| 설명 | 빈 DB에 스키마 재적용 테스트 |
| 완료 기준 | 오류 없이 모든 테이블/인덱스 생성 |

---

## Phase 1.5a: PostgreSQL 설계 문서 (Day 2-3)

**목표**: PostgreSQL 개념적 설계 문서 생성
**선행 조건**: Phase 1 완료
**후행 작업**: Phase 3

| ID | Task | 상태 | 선행 | 시작일 | 완료일 |
|----|------|------|------|--------|--------|
| 1.5a.1 | `db_refactoring/scripts/generate_docs.py` 작성 | ✅ COMPLETED | 1.5 | 2025-12-13 | 2025-12-13 |
| 1.5a.2 | `db_refactoring/docs/DATA_DICTIONARY.md` 자동 생성 | ✅ COMPLETED | 1.5a.1 | 2025-12-13 | 2025-12-13 |
| 1.5a.3 | `db_refactoring/docs/RELATIONSHIP_MAP.md` 자동 생성 | ✅ COMPLETED | 1.5a.1 | 2025-12-13 | 2025-12-13 |
| 1.5a.4 | `db_refactoring/docs/ER_DIAGRAM.md` 수동 작성 | ✅ COMPLETED | 1.5 | 2025-12-13 | 2025-12-13 |
| 1.5a.5 | `db_refactoring/docs/DOMAIN_MODEL.md` 수동 작성 | ✅ COMPLETED | 1.5 | 2025-12-13 | 2025-12-13 |

### Task 상세

#### 1.5a.1 `db_refactoring/scripts/generate_docs.py` 작성

| 항목 | 내용 |
|------|------|
| 설명 | DB 스키마에서 데이터 딕셔너리, 관계 문서 자동 생성 스크립트 |
| 완료 기준 | 스크립트 실행 시 문서 자동 생성 |

#### 1.5a.2-3 자동 생성 문서

| 항목 | 내용 |
|------|------|
| 설명 | generate_docs.py 실행하여 DATA_DICTIONARY.md, RELATIONSHIP_MAP.md 생성 |
| 완료 기준 | 15개 테이블 모두 문서화 |

#### 1.5a.4 `db_refactoring/docs/ER_DIAGRAM.md` 수동 작성

| 항목 | 내용 |
|------|------|
| 설명 | Mermaid 기반 E-R 다이어그램 작성 |
| 완료 기준 | 전체 구조 + 도메인별 상세 다이어그램 |

#### 1.5a.5 `db_refactoring/docs/DOMAIN_MODEL.md` 수동 작성

| 항목 | 내용 |
|------|------|
| 설명 | 비즈니스 도메인 설명 문서 |
| 완료 기준 | 보험/문서/매핑 도메인 설명 완료 |

---

## Phase 1.5b: Neo4j 설계 문서 (Day 2-3)

**목표**: Neo4j 그래프 개념적 설계 문서 생성
**선행 조건**: Phase 1 완료
**후행 작업**: Phase 4

| ID | Task | 상태 | 선행 | 시작일 | 완료일 |
|----|------|------|------|--------|--------|
| 1.5b.1 | `db_refactoring/docs/GRAPH_SCHEMA.md` 작성 | ✅ COMPLETED | 1.5 | 2025-12-13 | 2025-12-13 |
| 1.5b.2 | `db_refactoring/docs/NODE_DICTIONARY.md` 작성 | ✅ COMPLETED | 1.5 | 2025-12-13 | 2025-12-13 |
| 1.5b.3 | `db_refactoring/docs/RELATIONSHIP_DICTIONARY.md` 작성 | ✅ COMPLETED | 1.5 | 2025-12-13 | 2025-12-13 |
| 1.5b.4 | `db_refactoring/docs/GRAPH_PATTERNS.md` 작성 | ✅ COMPLETED | 1.5b.2, 1.5b.3 | 2025-12-13 | 2025-12-13 |
| 1.5b.5 | `db_refactoring/docs/ONTOLOGY_GAP_ANALYSIS.md` 작성 | ✅ COMPLETED | 1.5b.1 | 2025-12-13 | 2025-12-13 |

### Task 상세

#### 1.5b.1 `db_refactoring/docs/GRAPH_SCHEMA.md` 작성

| 항목 | 내용 |
|------|------|
| 설명 | Neo4j 그래프 모델 개요, 설계 원칙, 현재 상태 (640노드, 623관계) |
| 완료 기준 | 그래프 모델 설계 문서 완성 |

#### 1.5b.2 `db_refactoring/docs/NODE_DICTIONARY.md` 작성

| 항목 | 내용 |
|------|------|
| 설명 | 8개 노드 레이블별 속성 정의 (Company, Product, Coverage, Benefit 등) |
| 완료 기준 | 모든 노드 속성 문서화 |

#### 1.5b.3 `db_refactoring/docs/RELATIONSHIP_DICTIONARY.md` 작성

| 항목 | 내용 |
|------|------|
| 설명 | 6개 관계 타입 정의 (HAS_PRODUCT, OFFERS, HAS_BENEFIT 등) |
| 완료 기준 | 모든 관계 카디널리티, 방향성 문서화 |

#### 1.5b.4 `db_refactoring/docs/GRAPH_PATTERNS.md` 작성

| 항목 | 내용 |
|------|------|
| 설명 | 자주 사용되는 Cypher 쿼리 패턴 |
| 완료 기준 | 기본 조회, 계층 탐색, 비교 분석 패턴 문서화 |

#### 1.5b.5 `db_refactoring/docs/ONTOLOGY_GAP_ANALYSIS.md` 작성

| 항목 | 내용 |
|------|------|
| 설명 | ontology_concept.md 대비 현재 구현 갭 분석 |
| 완료 기준 | 미구현 노드(RiskEvent, Condition 등), 미구현 관계 목록화 |

---

## Phase 2: Alembic 마이그레이션 도입 (Day 4-5)

**목표**: Alembic을 사용한 마이그레이션 버전 관리 체계 수립
**선행 조건**: Phase 1 완료
**후행 작업**: Phase 3

| ID | Task | 상태 | 선행 | 시작일 | 완료일 |
|----|------|------|------|--------|--------|
| 2.1 | Alembic 설치 | ✅ COMPLETED | 1.5 | 2025-12-13 | 2025-12-13 |
| 2.2 | Alembic 초기화 (`alembic init`) | ✅ COMPLETED | 2.1 | 2025-12-13 | 2025-12-13 |
| 2.3 | `alembic.ini` 설정 | ✅ COMPLETED | 2.2 | 2025-12-13 | 2025-12-13 |
| 2.4 | `env.py` 환경변수 설정 | ✅ COMPLETED | 2.3 | 2025-12-13 | 2025-12-13 |
| 2.5 | 초기 마이그레이션 생성 (001_initial) | ✅ COMPLETED | 2.4 | 2025-12-13 | 2025-12-13 |
| 2.6 | 예시 마이그레이션 생성 (002_example) | ✅ COMPLETED | 2.5 | 2025-12-13 | 2025-12-13 |
| 2.7 | 마이그레이션 테스트 (upgrade/downgrade) | ✅ COMPLETED | 2.6 | 2025-12-13 | 2025-12-13 |

### Task 상세

#### 2.1 Alembic 설치

| 항목 | 내용 |
|------|------|
| 명령어 | `pip install alembic` |
| 완료 기준 | `alembic --version` 실행 성공 |

#### 2.2-2.4 Alembic 초기화 및 설정

| 항목 | 내용 |
|------|------|
| 명령어 | `cd db_refactoring && alembic init migrations` |
| 설정 | `alembic.ini`에 sqlalchemy.url 설정, `env.py`에 환경변수 로드 |

#### 2.5-2.6 마이그레이션 파일 생성

| 항목 | 내용 |
|------|------|
| 설명 | 초기 스키마 및 기존 SQL 마이그레이션을 Alembic 형식으로 변환 |
| 완료 기준 | `alembic upgrade head` 성공 |

#### 2.7 마이그레이션 테스트

| 항목 | 내용 |
|------|------|
| 설명 | upgrade/downgrade 양방향 테스트 |
| 완료 기준 | 롤백 후 재적용 성공 |

---

## Phase 2.5: DB 전환 (Test → Production) ✅ 완료

**목표**: 테스트 DB(15432)를 새 운영 DB로 전환
**선행 조건**: Phase 2 완료
**후행 작업**: Phase 3
**참조 문서**: [task_upgrade.md](./task_upgrade.md)

| ID | Task | 상태 | 선행 | 시작일 | 완료일 |
|----|------|------|------|--------|--------|
| 2.5.1 | `.env` 파일 생성 및 환경 설정 | ✅ COMPLETED | 2.7 | 2025-12-13 | 2025-12-13 |
| 2.5.2 | 하드코딩 URL 제거 (16개 파일) | ✅ COMPLETED | 2.5.1 | 2025-12-13 | 2025-12-13 |
| 2.5.3 | `load_dotenv()` 표준화 | ✅ COMPLETED | 2.5.1 | 2025-12-13 | 2025-12-13 |
| 2.5.4 | PostgreSQL 데이터 마이그레이션 | ✅ COMPLETED | 2.5.2 | 2025-12-13 | 2025-12-13 |
| 2.5.5 | Neo4j 동기화 (graph_loader.py) | ✅ COMPLETED | 2.5.4 | 2025-12-13 | 2025-12-13 |
| 2.5.6 | 연결 테스트 및 검증 | ✅ COMPLETED | 2.5.5 | 2025-12-13 | 2025-12-13 |

### Task 상세

#### 2.5.1 `.env` 파일 생성

| 항목 | 내용 |
|------|------|
| 설명 | 프로젝트 루트에 `.env` 파일 생성, 새 DB 연결 정보 정의 |
| 결과 | PostgreSQL: 15432, Neo4j: 17687 |

#### 2.5.2 하드코딩 URL 제거

| 항목 | 내용 |
|------|------|
| 설명 | `localhost:5432` fallback 제거, 환경변수 필수화 |
| 수정 파일 | 16개 (ingestion/6, vector_index/2, retrieval/2, ontology/1, api/3, scripts/2) |

#### 2.5.3 `load_dotenv()` 표준화

| 항목 | 내용 |
|------|------|
| 설명 | 모든 파일에 `from dotenv import load_dotenv; load_dotenv()` 추가 |
| 완료 기준 | 환경변수 없을 시 명확한 에러 발생 |

#### 2.5.4 PostgreSQL 데이터 마이그레이션

| 항목 | 내용 |
|------|------|
| 설명 | pg_dump → 새 DB 적용 (임베딩 제외) |
| 결과 | document: 38, document_clause: 134,844, coverage: 363, benefit: 357 |

#### 2.5.5 Neo4j 동기화

| 항목 | 내용 |
|------|------|
| 설명 | `python -m ingestion.graph_loader --all` 실행 |
| 결과 | 918 nodes, 949 relationships |

#### 2.5.6 연결 테스트

| 항목 | 내용 |
|------|------|
| 설명 | PostgreSQL, Neo4j 연결 및 데이터 검증 |
| 결과 | 모든 연결 정상, 데이터 일치 확인 |

### 새 운영 환경

| 항목 | 값 |
|------|-----|
| PostgreSQL URL | `postgresql://postgres:postgres@127.0.0.1:15432/insurance_ontology_test` |
| Neo4j URI | `bolt://127.0.0.1:17687` |
| 환경 설정 파일 | `.env` |

### 남은 선택 작업

| Task | 명령어 | 설명 |
|------|--------|------|
| 임베딩 재생성 | `python -m vector_index.build_index` | 1.8GB, ~30분 소요 |
| QA 테스트 | `python scripts/evaluate_qa.py` | 86% 정확도 목표 |

---

## Phase 3: 스크립트 및 검증 도구 (Day 6-7) ✅ 완료

**목표**: DB 초기화, 마이그레이션, 스키마 검증 스크립트 작성
**선행 조건**: Phase 1.5a, Phase 2 완료
**후행 작업**: Phase 6

| ID | Task | 상태 | 선행 | 시작일 | 완료일 |
|----|------|------|------|--------|--------|
| 3.1 | `db_refactoring/scripts/init_db.sh` 작성 | ✅ COMPLETED | 2.7 | 2025-12-13 | 2025-12-13 |
| 3.2 | `db_refactoring/scripts/verify_schema.py` 작성 | ✅ COMPLETED | 1.5a.5 | 2025-12-13 | 2025-12-13 |
| 3.3 | `db_refactoring/scripts/migrate.sh` 작성 | ✅ COMPLETED | 2.7 | 2025-12-13 | 2025-12-13 |
| 3.4 | 로컬 환경 전체 테스트 | ✅ COMPLETED | 3.1, 3.2, 3.3 | 2025-12-13 | 2025-12-13 |

### Task 상세

#### 3.1 `db_refactoring/scripts/init_db.sh` 작성

| 항목 | 내용 |
|------|------|
| 설명 | 스키마 생성 + 시드 데이터 적재 + Alembic 스탬프 |
| 완료 기준 | 빈 DB에서 전체 초기화 성공 |

#### 3.2 `db_refactoring/scripts/verify_schema.py` 작성

| 항목 | 내용 |
|------|------|
| 설명 | 현재 DB 스키마가 정의된 스키마와 일치하는지 검증 |
| 완료 기준 | 15개 테이블, 52개 인덱스, pgvector 확장 검증 |

#### 3.3 `db_refactoring/scripts/migrate.sh` 작성

| 항목 | 내용 |
|------|------|
| 설명 | upgrade/downgrade/history/current 명령 래퍼 |
| 완료 기준 | 모든 마이그레이션 명령 동작 |

#### 3.4 로컬 환경 전체 테스트

| 항목 | 내용 |
|------|------|
| 설명 | init_db.sh → verify_schema.py → migrate.sh 순차 실행 |
| 완료 기준 | 전체 워크플로우 오류 없음 |

---

## Phase 4: Neo4j 스키마 관리 (Day 8-9) ✅ 완료

**목표**: Neo4j 그래프 스키마 정의 및 PostgreSQL 동기화 검증
**선행 조건**: Phase 1.5b 완료
**후행 작업**: Phase 7

| ID | Task | 상태 | 선행 | 시작일 | 완료일 |
|----|------|------|------|--------|--------|
| 4.1 | `db_refactoring/neo4j/001_graph_schema.cypher` 생성 | ✅ COMPLETED | 1.5b.3 | 2025-12-13 | 2025-12-13 |
| 4.2 | `db_refactoring/scripts/init_neo4j.sh` 작성 | ✅ COMPLETED | 4.1 | 2025-12-13 | 2025-12-13 |
| 4.3 | `db_refactoring/scripts/verify_neo4j_sync.py` 작성 | ✅ COMPLETED | 4.1 | 2025-12-13 | 2025-12-13 |
| 4.4 | PostgreSQL ↔ Neo4j 동기화 검증 실행 | ✅ COMPLETED | 4.3 | 2025-12-13 | 2025-12-13 |
| 4.5 | `graph_loader.py`에 PARENT_OF 관계 추가 | ✅ COMPLETED | 4.4 | 2025-12-13 | 2025-12-13 |

### Task 상세

#### 4.1 `db_refactoring/neo4j/001_graph_schema.cypher` 생성

| 항목 | 내용 |
|------|------|
| 설명 | 제약조건 (UNIQUE), 인덱스, 노드/관계 스키마 정의 |
| 완료 기준 | cypher-shell로 실행 가능 |

#### 4.2 `db_refactoring/scripts/init_neo4j.sh` 작성

| 항목 | 내용 |
|------|------|
| 설명 | Neo4j 초기화 + graph_loader.py 호출 |
| 완료 기준 | 빈 Neo4j에서 전체 동기화 성공 |

#### 4.3 `db_refactoring/scripts/verify_neo4j_sync.py` 작성

| 항목 | 내용 |
|------|------|
| 설명 | PostgreSQL과 Neo4j 간 엔티티 수 일치 검증 |
| 완료 기준 | 5개 엔티티 타입 수량 일치 확인 |

#### 4.4 동기화 검증 실행

| 항목 | 내용 |
|------|------|
| 설명 | verify_neo4j_sync.py 실행 |
| 완료 기준 | Company(8), Product(8), Coverage(363), Benefit(357) 등 일치 |

#### 4.5 (선택) PARENT_OF 관계 추가

| 항목 | 내용 |
|------|------|
| 설명 | graph_loader.py에 Coverage 계층 관계 추가 |
| 완료 기준 | 52개 자식 담보에 PARENT_OF 관계 생성 |

---

## Phase 5: pgvector 인덱스 관리 (Day 10-11)

**목표**: pgvector 인덱스 설정 문서화 및 성능 모니터링 도구 작성
**선행 조건**: Phase 1 완료 (독립적 진행 가능)
**후행 작업**: 없음

| ID | Task | 상태 | 선행 | 시작일 | 완료일 |
|----|------|------|------|--------|--------|
| 5.1 | `db_refactoring/postgres/003_pgvector_setup.sql` 생성 | ⬜ PENDING | 1.5 | - | - |
| 5.2 | HNSW 인덱스 파라미터 가이드 문서화 | ⬜ PENDING | 5.1 | - | - |
| 5.3 | `db_refactoring/scripts/monitor_vector_search.py` 작성 | ⬜ PENDING | 5.1 | - | - |
| 5.4 | 임베딩 모델 변경 마이그레이션 템플릿 작성 | ⬜ PENDING | 5.2 | - | - |
| 5.5 | 벡터 검색 벤치마크 실행 및 기준선 수립 | ⬜ PENDING | 5.3 | - | - |

### Task 상세

#### 5.1 `db_refactoring/postgres/003_pgvector_setup.sql` 생성

| 항목 | 내용 |
|------|------|
| 설명 | pgvector 확장 설치, 벡터 테이블, HNSW 인덱스 정의 |
| 완료 기준 | 독립 실행 가능한 pgvector 설정 파일 |

#### 5.2 HNSW 인덱스 파라미터 가이드

| 항목 | 내용 |
|------|------|
| 설명 | m, ef_construction, ef_search 파라미터 튜닝 가이드 |
| 완료 기준 | 데이터 규모별 권장값 문서화 |

#### 5.3 `db_refactoring/scripts/monitor_vector_search.py` 작성

| 항목 | 내용 |
|------|------|
| 설명 | 벡터 검색 벤치마크 및 인덱스 통계 조회 |
| 완료 기준 | 검색 지연시간, recall 측정 기능 |

#### 5.4 임베딩 모델 변경 마이그레이션 템플릿

| 항목 | 내용 |
|------|------|
| 설명 | 임베딩 모델 변경 시 절차 템플릿 |
| 완료 기준 | 단계별 체크리스트 문서화 |

#### 5.5 벡터 검색 벤치마크

| 항목 | 내용 |
|------|------|
| 설명 | 현재 성능 측정 및 기준선 기록 |
| 완료 기준 | 134,644건 기준 검색 성능 기록 |

---

## Phase 6: CI/CD 통합 (Day 12)

**목표**: GitHub Actions를 통한 자동 스키마 검증
**선행 조건**: Phase 3 완료
**후행 작업**: 없음

| ID | Task | 상태 | 선행 | 시작일 | 완료일 |
|----|------|------|------|--------|--------|
| 6.1 | `.github/workflows/schema-check.yml` 작성 | ⬜ PENDING | 3.4 | - | - |
| 6.2 | PR 테스트 (자동 검증 확인) | ⬜ PENDING | 6.1 | - | - |
| 6.3 | 마이그레이션 가이드 문서 작성 | ⬜ PENDING | 6.2 | - | - |

### Task 상세

#### 6.1 GitHub Actions 워크플로우 작성

| 항목 | 내용 |
|------|------|
| 설명 | pgvector 이미지 기반 테스트 환경에서 스키마 검증 |
| 완료 기준 | db/, ingestion/ 변경 시 자동 실행 |

#### 6.2 PR 테스트

| 항목 | 내용 |
|------|------|
| 설명 | 테스트 PR 생성하여 워크플로우 동작 확인 |
| 완료 기준 | 성공/실패 케이스 모두 검증 |

#### 6.3 마이그레이션 가이드 문서

| 항목 | 내용 |
|------|------|
| 설명 | 개발자용 마이그레이션 작성/적용 가이드 |
| 완료 기준 | README 또는 별도 문서 작성 |

---

## Phase 7: 온톨로지 확장 (Day 13-15) ✅ 완료

**목표**: ontology_concept.md 기반 Neo4j 스키마 확장
**선행 조건**: Phase 4 완료
**후행 작업**: 없음

| ID | Task | 상태 | 선행 | 시작일 | 완료일 |
|----|------|------|------|--------|--------|
| 7.1 | PostgreSQL에 `risk_event` 테이블 설계 | ✅ COMPLETED | 4.4 | 2025-12-13 | 2025-12-13 |
| 7.2 | PostgreSQL에 `condition` 테이블 설계 | ✅ COMPLETED (기존) | 4.4 | - | - |
| 7.3 | PostgreSQL에 `exclusion` 테이블 설계 | ✅ COMPLETED (기존) | 4.4 | - | - |
| 7.4 | Alembic 마이그레이션 작성 (ontology_extension) | ✅ COMPLETED | 7.1 | 2025-12-13 | 2025-12-13 |
| 7.5 | Neo4j에 RiskEvent 노드 추가 | ✅ COMPLETED | 7.4 | 2025-12-13 | 2025-12-13 |
| 7.6 | Neo4j에 Condition 노드 추가 | ✅ COMPLETED | 7.4 | 2025-12-13 | 2025-12-13 |
| 7.7 | Neo4j에 Exclusion 노드 추가 | ✅ COMPLETED | 7.4 | 2025-12-13 | 2025-12-13 |
| 7.8 | graph_loader.py 확장 | ✅ COMPLETED | 7.5, 7.6, 7.7 | 2025-12-13 | 2025-12-13 |
| 7.9 | (선택) Plan 노드 설계 및 추가 | ✅ COMPLETED | 7.8 | 2025-12-13 | 2025-12-13 |

### Task 상세

#### 7.1-7.3 새 테이블 설계

| 항목 | 내용 |
|------|------|
| risk_event | event_type, event_name, severity_level, icd_code_pattern |
| condition | condition_type, value, unit, description |
| exclusion | exclusion_type, description, clause_reference |

#### 7.4 Alembic 마이그레이션

| 항목 | 내용 |
|------|------|
| 설명 | 새 테이블 및 관계 생성 마이그레이션 |
| 완료 기준 | upgrade/downgrade 모두 작동 |

#### 7.5-7.7 Neo4j 노드 추가

| 항목 | 내용 |
|------|------|
| 설명 | RiskEvent, Condition, Exclusion 노드 및 관계 추가 |
| 관계 | TRIGGERS (Benefit→RiskEvent), HAS_CONDITION, HAS_EXCLUSION |

#### 7.8 graph_loader.py 확장

| 항목 | 내용 |
|------|------|
| 설명 | 새 엔티티 동기화 로직 추가 |
| 완료 기준 | --sync-risk-events, --sync-conditions, --sync-exclusions 옵션 |

#### 7.9 (선택) Plan 노드 ✅ 완료

| 항목 | 내용 |
|------|------|
| 설명 | 가입설계서 파싱 연동 후 Plan 노드 추가 |
| 완료 기준 | Plan → Coverage INCLUDES 관계 생성 |
| 완료 내용 | PostgreSQL: plan, plan_coverage 테이블, Neo4j: Plan 노드/인덱스, graph_loader.py sync_plans() |

---

## 진행 이력

| 날짜 | Task ID | 변경 내용 | 비고 |
|------|---------|----------|------|
| 2025-12-13 | - | task.md 파일 생성 | 초기 작업 계획 수립 |
| 2025-12-13 | - | task.md 재작성 | database_refactoring.md v2.2 기반, 선후관계 반영 |
| 2025-12-13 | 1.1-1.4 | Phase 1 대부분 완료 | 디렉토리 구조, 스키마 덤프, SQL 파일 생성 |
| 2025-12-13 | 1.5a.1-5 | Phase 1.5a 완료 | PostgreSQL 설계 문서 전체 완성 |
| 2025-12-13 | 1.5b.1-3 | Phase 1.5b 진행 중 | Neo4j 설계 문서 3/5 완료 |
| 2025-12-13 | 1.5b.4-5 | Phase 1.5b 완료 | GRAPH_PATTERNS.md, ONTOLOGY_GAP_ANALYSIS.md 작성 |
| 2025-12-13 | 1.5 | Phase 1 완료 | 스키마 검증 (15 테이블, 52 인덱스, pgvector) |
| 2025-12-13 | - | 설계-구현 연결 관계 추가 | ER_DIAGRAM.md, GRAPH_SCHEMA.md, task.md, docs/README.md |
| 2025-12-13 | 2.1-2.7 | Phase 2 완료 | Alembic 초기화, baseline + 예시 마이그레이션, upgrade/downgrade 테스트 |
| 2025-12-13 | 2.5.1-6 | Phase 2.5 완료 | DB 전환 작업 (Test→Production), 16개 파일 수정, 데이터 마이그레이션, Neo4j 동기화 |
| 2025-12-13 | 4.1-4.5 | Phase 4 완료 | Neo4j 스키마 관리: 001_graph_schema.cypher, init_neo4j.sh, verify_neo4j_sync.py, PARENT_OF 관계 추가 (52개) |
| 2025-12-13 | 7.1-7.8 | Phase 7 완료 | 온톨로지 확장: risk_event 테이블, Alembic 마이그레이션, Neo4j 스키마 확장, graph_loader.py 확장 |
| 2025-12-13 | 7.9 | Plan 노드 완료 | plan/plan_coverage 테이블, Neo4j Plan 노드/인덱스, sync_plans() 추가 |
| 2025-12-13 | 3.1-3.4 | Phase 3 완료 | init_db.sh, verify_schema.py, migrate.sh 작성 및 테스트 |

---

## 실패한 Task 기록

> 실패한 작업이 발생하면 아래 형식으로 기록합니다.

<!--
### [Task ID] Task 이름

**실패 일시**: YYYY-MM-DD HH:MM
**실패 원인**:
- 원인 1
- 원인 2

**시도한 해결 방법**:
1. 해결 시도 1 → 결과
2. 해결 시도 2 → 결과

**최종 해결 방법**:
- 해결 방법 설명

**교훈**:
- 향후 유사 문제 방지를 위한 교훈
-->

---

## 일정 요약

| Phase | 작업 | 기간 | 선행 조건 | 상태 |
|-------|------|------|----------|------|
| Phase 1 | 스키마 추출 | Day 1-2 | - | ✅ 완료 |
| Phase 1.5a | PostgreSQL 설계 문서 | Day 2-3 | Phase 1 | ✅ 완료 |
| Phase 1.5b | Neo4j 설계 문서 | Day 2-3 | Phase 1 | ✅ 완료 |
| Phase 2 | Alembic 도입 | Day 4-5 | Phase 1 | ✅ 완료 |
| Phase 2.5 | DB 전환 (Test→Production) | Day 5 | Phase 2 | ✅ 완료 |
| Phase 3 | 스크립트/검증 도구 | Day 6-7 | Phase 1.5a, 2 | ✅ 완료 |
| Phase 4 | Neo4j 스키마 | Day 8-9 | Phase 1.5b | ✅ 완료 |
| Phase 5 | pgvector 관리 | Day 10-11 | Phase 1 | ⬜ |
| Phase 6 | CI/CD 통합 | Day 12 | Phase 3 | ⬜ |
| Phase 7 | 온톨로지 확장 | Day 13-15 | Phase 4 | ✅ 완료 |

---

## 참고 문서

- [database_refactoring.md](./database_refactoring.md) - 리팩토링 상세 계획
- [database_status.md](./database_status.md) - 현재 DB 상태
- [ontology_concept.md](./design/ontology_concept.md) - 온톨로지 개념 설계
- [graph_loader.py](../ingestion/graph_loader.py) - Neo4j 동기화 구현체

---

**문서 버전**: 2.1
**최종 수정**: 2025-12-13
**변경 이력**:
- v1.0 (2025-12-13): 초기 작성
- v2.0 (2025-12-13): database_refactoring.md v2.2 기반 재작성, 선후관계 반영, Phase 1.5를 a/b로 분리
- v2.1 (2025-12-13): Phase 2.5 (DB 전환) 추가, 16개 파일 수정, 데이터 마이그레이션 완료
